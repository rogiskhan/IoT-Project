...
class telegram(object):
    ...
    def start(self): #controllo del messaggio start
        ...
        if len(response)==0: #in ogni caso considero il caso in cui non ci sono i messaggi. Da getUpdates ho una lista di dizionari(ogni dizionario è un messaggio con i suoi parametri)
            pass
            ...
            if response[len(response)-1]['message']['text']=='Start':
                         ...
                         self.ini=int(response[len(response)-1]['update_id']) #aggiorno il self.ini in modo che posso continuare ad aspettare i messaggi
                         #lo aggiorno ora perché mi serve solo prima dello sta bene così
                         return self.chat_id
    ...
    def waiting(self): #loop di avvio. appena si avvia il servizio aspetta lo start
        self.is_condition=None
        print('waiting...\n')
        while self.is_condition==None:
            self.msg()
    ...
    def msg(self): #controllo del messaggio stop
        if self.ini==0: #se non ci sono messaggi inviati non serve aumentare l'offset
            response = self.bot.getUpdates()
        ...
        if len(response)==0: #in ogni caso considero il caso in cui non ci sono i messaggi. Da getUpdates ho una lista di dizionari(ogni dizionario è un messaggio con i suoi parametri)
            ...
            if response[len(response)-1]['message']['text']=='Stop':
                         self.is_condition='S'
                         
            elif response[len(response)-1]['message']['text']=='Piano':
                         self.is_condition='P'
                         
            elif response[len(response)-1]['message']['text']=='Consigli':
                         self.is_condition='C'
